...
- Integration shape cutover (P4.3): `TurnEvent` is a discriminated per-agent union — every serialized turn requires `"agent"`; undiscriminated dict turns and `extra` fields are rejected. See `docs/migration/history.md`.
- Integration shape cutover (P4.3): `GSM8K`, `MATH`, and `AlfWorld` subclass spine `Dataset` with lazy splits and seeds; `MATH.metric` removed — use `math_metric` from `dspy.integrations.datasets.metrics`; `gsm8k_metric` uses `(example, pred, trace)`.
- Integration shape cutover (P4.3): `DrLlmPoolLM` accepts optional `session_id_resolver=` (`PoolSessionIdResolver`); default remains `resolve_pool_session_id` (log-session lookup).
- Call-log centralization (P1.2): removed `Module.inspect_call_log` and `BaseLM.inspect_call_log` (use `run.inspect_call_log()` / `run.read_call_log()` or `pretty_print_call_log(scope.call_log)`); deleted `dspy.runtime.run_log` (paths/redaction/session split into `log_paths`, `log_redaction`, `run_log_session`; fan-out and disk append in `dspy.runtime.call_log.coordinator`).
- Transparency and call-options cutover (P1.1): `dspy/runtime/transparency.py` split into `dspy/runtime/transparency/{types,resolve,validate,report}.py`; `validate_compiled_call` renamed to `enforce_compiled_call_transparency`; `resolve_lm_config` split into `merge_call_config` and `trace_config_provenance`; `resolve_adapter` renamed to `require_adapter`; `LM_CONFIG_PROVENANCE_FIELDS` moved to `dspy.runtime.transparency.resolve`; `ModuleCallOptions` moved to `dspy.runtime.call_options`; `PredictOptions` moved to `dspy.predict.call_options` (removed from `dspy.core.types`).
- Clients and finetune cutover (P0.4): generic LM error extraction moved to `dspy/clients/errors.py`; finetune provider lookup is lazy in `dspy/clients/finetune/registry.py`; finetune lifecycle moved to `FinetuneService` (`dspy.clients.finetune`); inference `LM` no longer accepts `provider=` or exposes `launch`/`kill`/`finetune`/`reinforce`/`infer_provider`; `LM` serialized state no longer includes `finetuning_model`, `launch_kwargs`, or `train_kwargs`; removed `dspy.clients.finetune.lm`. See `docs/migration/finetune.md`.
