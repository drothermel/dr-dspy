...
def test_run_repair_command_apply_uses_planned_repair() -> None:
    backend = MagicMock()
    plan = RepairPlan(
        generation_retry_prediction_ids=["p1"],
        generation_retry_summary=RepairRetrySummary(recoverable_count=1),
    )
    backend.build_repair_plan.return_value = plan
    backend.apply_repair.return_value = RepairApplyResult(
        repair_token="repair",
        stranded_generations_marked=0,
        generation_retries_enqueued=1,
        generation_retries_reset=1,
        stranded_scoring_marked=0,
        pending_scoring_enqueued=0,
        scoring_retries_enqueued=0,
        scoring_retries_marked_queued=0,
    )
    flow.run_repair_command(
        backend,
        config=_config(),
    ...
    backend.apply_repair.assert_called_once_with(
        _config(),
        experiment_name="exp",
        generation_limit=10,
        scoring_limit=10,
