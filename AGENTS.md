...
2. **Integrations:** `dspy.integrations.*` (optional extras: `mcp`, `langchain`, `optuna`, `gepa`; import vendor modules directly, e.g. `dspy.integrations.finetune.openai`, `dspy.integrations.retrieval.weaviate`, `dspy.integrations.optimizers.gepa.adapter`)
