...
from dspy.history.discovery import _is_repl_history_type, is_agent_history_type, is_conversation_turn_log_type
...
def test_is_repl_history_type_detects_repl_history():
    assert _is_repl_history_type(REPLHistory)
    assert _is_repl_history_type(Optional[REPLHistory])
