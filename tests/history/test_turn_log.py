...
def test_turn_log_rejects_missing_agent_discriminator():
    with pytest.raises(ValueError, match="agent"):
        TurnLog.model_validate({"turns": [{"thought": "legacy"}]})
...
def test_turn_log_serialization_round_trip_with_nested_tool_calls():
    ...
    history = TurnLog(turns=(ReActV2TurnEvent(tool_calls=tool_calls),))
