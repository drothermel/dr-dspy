...
from dspy.core.types import LMOutputBuilder, LMStreamDeltaEvent, LMTextDelta, LMThinkingDelta, LMToolCallDelta
...
def test_stream_builder_rejects_incomplete_tool_call_arguments():
    ...
    builder.apply(
        LMStreamDeltaEvent(
            output_index=0, part_index=0, delta=LMToolCallDelta(id="call_1", name="search", args_delta='{"query": ')
        )
    )
