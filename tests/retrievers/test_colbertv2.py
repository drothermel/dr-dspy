...
from dspy.retrievers.colbertv2 import colbertv2_get_request, colbertv2_post_request
...
def test_get_request_raises_on_server_error():
    ...
    with (
        patch("dspy.retrievers.colbertv2.requests.get", return_value=mock_response),
...
def test_post_request_raises_on_server_error():
    ...
    with (
        patch("dspy.retrievers.colbertv2.requests.post", return_value=mock_response),
...
def test_get_request_success():
    ...
    with patch("dspy.retrievers.colbertv2.requests.get", return_value=mock_response):
...
def test_post_request_success():
    mock_response = MagicMock()
    mock_response.json.return_value = {"topk": [{"text": "doc1", "score": 0.9}]}
    with patch("dspy.retrievers.colbertv2.requests.post", return_value=mock_response):
