...
def test_grpo_dataset_shuffler():
    dataset = [1, 2, 3]
    grpo = GRPO(num_dspy_examples_per_grpo_step=3, exclude_demos=True)
    trainset_instances = []
    for i in range(4):
...
def test_grpo_dataset_shuffler_with_num_ex_per_step_less_dataset():
    dataset = [1, 2, 3]
    grpo = GRPO(num_dspy_examples_per_grpo_step=2, exclude_demos=True)
    trainset_instances = []
    for i in range(15):
        trainset_instances.append(
            grpo.select_training_sample_and_update_shuffled_trainset(original_trainset=dataset, train_step_idx=i)
...
def test_grpo_dataset_shuffler_with_num_ex_per_step_greater_dataset():
    dataset = [1, 2, 3]
    grpo = GRPO(num_dspy_examples_per_grpo_step=5, exclude_demos=True)
    trainset_instances = []
    for i in range(6):
        trainset_instances.append(
            grpo.select_training_sample_and_update_shuffled_trainset(original_trainset=dataset, train_step_idx=i)
