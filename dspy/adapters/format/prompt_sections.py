...
from dspy.adapters.prompt_format import format_field_value, get_field_spec_description_string
...
def format_field_description(task_spec: TaskSpec) -> str:
    return (
...
def format_field_structure_header(field_formatter: FieldFormatter, task_spec: TaskSpec) -> str:
    return build_field_structure_instructions(
        input_section=build_role_field_sections(field_formatter, task_spec, FieldRole.INPUT),
        output_section=build_role_field_sections(field_formatter, task_spec, FieldRole.OUTPUT),
        completed_marker=COMPLETED_MARKER,
    )


def format_task_description(task_spec: TaskSpec) -> str:
...
def header_user_message_output_requirements(task_spec: TaskSpec) -> str:
...
def format_header_user_message_content(
    ...
    *,
    ...
    output_requirements_fn: Any = header_user_message_output_requirements,
) -> str | list[dict[str, Any]]:
    if inputs_include_multimodal_custom_type_values(task_spec=task_spec, inputs=inputs):
        output_requirements = output_requirements_fn(task_spec) if main_request else None
    ...
    messages = [prefix]
    field_sections = field_formatter.format_field_with_value(
        {
    ...
    if main_request:
        output_requirements = output_requirements_fn(task_spec)
...
def format_header_assistant_message_content(
    field_formatter: FieldFormatter,
    task_spec: TaskSpec,
    outputs: dict[str, Any],
    missing_field_message: str | None = None,
) -> str:
    assistant_message_content = field_formatter.format_field_with_value(
        {
...
def format_header_finetune_data(
    adapter: Adapter,
    field_formatter: FieldFormatter,
    task_spec: TaskSpec,
    ...
    assistant_message_content = format_header_assistant_message_content(
