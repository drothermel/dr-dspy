# Final Review: dr-dspy Updates

Source: `docs/final_review.md`. This split keeps findings that are owned by this repository or by the DSPy-facing dr-llm integration layer.
...
Impact: concurrent `Parallel` runs can interleave and drop entries in shared call logs. `run.inspect_call_log()` is safer than shared LM/module logs under concurrency, but disk-session collision remains a separate risk.
...
## Predict, Sampling, and Evaluation
...
## Adapter, Integration, and Persistence
...
## dr-llm Integration Work Owned by dr-dspy

### Pool acquisition session identity has unsafe defaults
...
### dr-llm LMs cannot be loaded through the safe state path
...
### Last-week compression model controls are only partially expressible through `DrLlmLM`

The observed `nl_latents` compression curves used provider-specific reasoning and thinking controls. Current dr-dspy can approximate ordinary sampling with `temperature` and per-call `LMConfig(top_p=0.95)`, but it cannot carry OpenRouter reasoning-off controls, OpenRouter reasoning effort as provider-specific `BackendRequest.reasoning`, OpenAI minimal thinking, or Google thinking-off controls. Current mapping puts DSPy `reasoning.effort` on `BackendRequest.effort` and hard-codes `BackendRequest.reasoning=None`.
...
Impact: even when the model string matches, provider payloads and pool fingerprints are not exact unless dr-dspy can carry provider-specific dr-llm reasoning objects.

### Default `nl_latents` T1 configs are rejected through current dr-dspy mapping

The default T1 compression setup uses dr-llm-native `BackendRequest.reasoning` shapes: OpenRouter disabled reasoning, OpenRouter effort, and OpenAI minimal thinking. Focused validation through the dr-llm provider registry showed all five default T1 configs fail when represented through current dr-dspy mapping:
...
Impact: the current dr-dspy setup cannot carry exact default T1 requests through either `DrLlmDirectLM` or `DrLlmPoolLM`. dr-dspy needs an explicit dr-llm config/request path or provider-specific reasoning support before it can be used for this experiment family.

### `nl_latents` pool curves cannot be exactly reproduced through dr-dspy `Predict`

The `nl_latents` encoder/decoder pool prompts are raw single-user-message templates. Routing those calls through dr-dspy `Predict` changes the wire prompt by adding adapter/system scaffolding, field descriptions, field structure, output requirements, and field wrappers.
...
### `nl-code` DSPy reproduction needs an explicit port
...
### Pool LM lifecycle is easy to misuse
...
### dr-llm v1 scope blocks major DSPy modules
...
### Batch-fill pool workflow is not surfaced on `DrLlmPoolLM`

dr-llm's `PoolBackend` supports a batch flow with `submit_batch`, `await_drain`/`adrain`, and then acquire. `DrLlmPoolLM` exposes `aforward` and `acquire_samples`, but not `submit_batch` or drain methods.
...
### dr-llm prompt-model use breaks proposal paths that set `n`

The dr-llm contract rejects any non-`None` `config.n` but reports that `n>1` is unsupported. `LMConfig(n=1)` raises `LMUnsupportedFeatureError`. This matters for optimizers: MIPRO's grounded proposer and dataset-summary flows use single-completion `n=1`, while COPRO proposal calls use `n=breadth-1`.
...
Impact: compression-doc MIPRO and COPRO optimizer runs can use `DrLlmDirectLM` as the task LM only if proposal calls use another LM or this contract changes. Allowing `n=1` would fix some MIPRO paths; COPRO breadth still needs either multi-completion support or an emulated loop of single completions.

### Pool acquire aggregate provenance is dropped
...
### Pool fingerprint and metadata behavior is under-documented on the DSPy side
...
The larger experiment-control deltas are OpenRouter reasoning-off controls, GPT-5 minimal thinking, OpenRouter provider-specific effort controls, and suppressing provider-default sampling with explicit empty sampling controls.
...
Impact: these should be explicit for experiments that rely on custom provider registries, dr-llm's maximum-effort mode, or exact parity with `nl_latents`/`nl-code` catalog controls.
...
| dr-llm direct path | Missing full `Predict` + `JSONAdapter` + `DrLlmDirectLM` + `RunContext` happy-path test. |
| dr-llm state | Missing direct and pool `dump_state`/`load_state` round trips. |
| dr-llm pool acquire | Missing test that makes the no-session fallback behavior explicit. |
...
10. Fix dr-llm safe state loading, session identity, provider-specific reasoning/config mapping, and pool lifecycle behavior.
...
Existing targeted tests for reasoning requirements and run-log session creation passed, but they do not cover the failing cases above.

The dr-llm focused review ran:
...
- Postgres integration checks: skipped because no integration DSN was configured.
- Focused local scripts confirmed the safe state-load failure, shallow pool copy/double-close behavior, and `LMConfig(n=1)` rejection.

Experiment-parity checks not run: live provider calls, `nl-code` session replay, or `nl_latents` curve replay.
