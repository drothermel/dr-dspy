...
1. Delete or intentionally shim empty `_meta` and `transparency` package shells.
2. Break the `core` -> `clients` dependency.
3. Break the `task_spec` -> `adapters` dependency.
4. Move call validation and trace-running helpers down to spine/runtime modules.
5. Decouple client finetune entrypoints from integration providers.
6. Split transparency and rename LM config resolution responsibilities.
7. Relocate `PredictOptions`.
8. Centralize call-log fan-out and split run-log helpers.
9. Consolidate OpenAI and JSON serialization ownership.
10. Unify history truncation helpers and extract a shared agent loop skeleton.
11. Split `Module`, interpreter layout, and core type submodules.
12. Decompose the GRPO compile-loop structure.
13. Replace monkey-patched trace capture with an explicit wrapper boundary.
14. Split BaseLM logging/state serialization and normalize LM finetune lifecycle.
15. Put every live-provider test behind one opt-in marker policy.
16. Clarify whether `dspy.testing` is public API or repo-local test support.
17. Consolidate LM test doubles and reusable adapter formatting scenarios.
18. Unify optional dependency imports and normalize spine re-exports.
