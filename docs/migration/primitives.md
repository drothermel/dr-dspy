...
| `from dspy.primitives.code_interpreter import ...` | removed — use `from dspy.primitives import ...` or `dspy.primitives.python_interpreter.protocol` |
...
`Module` remains importable from `dspy.primitives`. Internals live in focused helpers:
`module_graph.py`, `module_copy.py`, `module_execution.py`, and `module_lm.py`.
Public `Module` methods are thin delegations to those modules.
...
- JSON-RPC application error codes are generated from `dspy/primitives/python_interpreter/jsonrpc_app_errors.json` via `scripts/generate_jsonrpc_errors.py`.
- Interpreter protocol types (`CodeInterpreter`, `CodeInterpreterError`, `FinalOutput`) live in `dspy.primitives.python_interpreter.protocol`.
- Host-side JSON-RPC response handling is centralized in `dspy.primitives.python_interpreter.pump`.
...
- Dot attribute access prefers API methods and properties; dynamic field names that collide with API names (for example `keys`, `get`, `fork`) are readable via bracket notation only.
